...
import { DiscoveredWallet } from '../types/recoveryPool';

// Bitcoin Core wallet.dat file parser
// The .dat file is a Berkeley DB file containing key-value pairs
// We'll parse the raw data to extract private keys and metadata

interface ParsedWalletEntry {
  type: 'key' | 'ckey' | 'wkey' | 'name' | 'version' | 'pool' | 'bestblock';
  data: any;
}

// Search for WIF-encoded private keys in raw binary data
function searchForWIFKeys(buffer: Buffer): string[] {
  const keys: string[] = [];
  const patterns = [
    // Bitcoin mainnet WIF (5, K, L prefixes)
    { regex: /[5KL][1-9A-HJ-NP-Za-km-z]{50,51}/g },
    // Bitcoin testnet WIF (9, c prefixes)
    { regex: /[9c][1-9A-HJ-NP-Za-km-z]{50,51}/g },
  ];
...
// Search for hex-encoded private keys
function searchForHexKeys(buffer: Buffer): string[] {
  const keys: string[] = [];
  // Look for 32-byte (64 hex chars) private keys
...
// Search for mnemonic seed phrases
function searchForMnemonics(buffer: Buffer): string[] {
  ...
  try {
    ...
    for (let len = 12; len <= 24; len += 3) {
      for (let i = 0; i <= words.length - len; i++) {
        const candidate = words.slice(i, i + len);
        const validCount = candidate.filter(w => wordSet.has(w)).length;
        if (validCount === candidate.length) {
          mnemonics.push(candidate.join(' '));
        }
      }
    }
  } catch {
...
export async function parseWalletDat(
  file: File,
  onProgress?: (progress: number, found: { type: string; data: string } | null) => void
): Promise<{
  wifKeys: string[];
  hexKeys: string[];
  mnemonics: string[];
  addresses: string[];
  rawKeys: DiscoveredWallet[];
}> {
  ...
  // Search for WIF keys
  const wifKeys = searchForWIFKeys(data);
  onProgress?.(30, { type: 'wif', data: `${wifKeys.length} keys found` });

  // Search for hex private keys
  const hexKeys = searchForHexKeys(data);
  onProgress?.(50, { type: 'hex', data: `${hexKeys.length} keys found` });

  // Search for mnemonic phrases
  const mnemonics = searchForMnemonics(data);
  onProgress?.(70, { type: 'mnemonic', data: `${mnemonics.length} phrases found` });
  ...
  try {
    ...
    const addrPattern = /(?:^|[^a-zA-Z0-9])([13][a-km-zA-HJ-NP-Z1-9]{25,34}|bc1[a-z0-9]{39,59}|tb1[a-z0-9]{39,59})(?:[^a-zA-Z0-9]|$)/g;
    let match;
    while ((match = addrPattern.exec(text)) !== null) {
      addresses.push(match[1]);
    }
    // Ethereum addresses (0x...)
    const ethPattern = /(?:^|[^a-fA-F0-9])(0x[a-fA-F0-9]{40})(?:[^a-fA-F0-9]|$)/g;
  ...
  onProgress?.(85, { type: 'address', data: `${addresses.length} addresses found` });

  // Convert WIF keys to discovered wallets
  const rawKeys: DiscoveredWallet[] = [];
  for (const wif of wifKeys) {
    try {
      const keyPair = bitcoin.ECPair.fromWIF(wif);
      const { address, pubkey } = bitcoin.payments.p2wpkh({ pubkey: keyPair.publicKey });
      if (address) {
        rawKeys.push({
          id: `wif-${wif.slice(0, 8)}-${Date.now()}`,
          network: 'Bitcoin',
          path: 'wallet.dat',
          address,
  ...
  onProgress?.(100, { type: 'complete', data: 'Parsing complete' });
